...
class GridSlidingWindow:
    def __init__(self):
        ...
        self.history_window_size = 20 # 최근 20개의 프레임을 사용하여 평활화
        
        self.local_path_pub = rospy.Publisher("local_path", Path, queue_size=1)
        rospy.Subscriber("occupancy_grid", OccupancyGrid, self.occupancy_grid_callback)

    def smooth_lane_points(self, history_list):
        #비어있지 않다면 마지막 프레임의 데이터 반환
        if history_list:
    ...
    def occupancy_grid_callback(self, msg):
        ...
        # history 업데이트: 현재 프레임의 데이터가 있으면 history에 추가
        if len(self.left_lane_points) > 0:
            self.left_lane_history.append(self.left_lane_points)
        if len(self.right_lane_points) > 0:
            self.right_lane_history.append(self.right_lane_points)

        # history 길이 제한
        if len(self.left_lane_history) > self.history_window_size:
            self.left_lane_history.pop(0)
        if len(self.right_lane_history) > self.history_window_size:
            self.right_lane_history.pop(0)

        # 현재 프레임이 empty라면 history를 사용하여 평활화된 lane point 확보
        if len(self.left_lane_points) == 0:
            smoothed_left = self.smooth_lane_points(self.left_lane_history)
        else:
            smoothed_left = self.left_lane_points

        if len(self.right_lane_points) == 0:
            smoothed_right = self.smooth_lane_points(self.right_lane_history)
        else:
    ...
    def sliding_window_lane_detection_region(self, img, region, window_width, window_height, min_pixel_threshold):
        ...
        bottom_indices = np.where(nonzero_region[:, 1] >= start_y - 5)[0]
        ...
        consecutive_empty = 0
                ...
                # while True:
                #     # 전체 이미지 기준의 min_y_occupied를 사용하므로 두 영역의 윈도우 쌓는 높이가 동일함
                ...
                #         consecutive_empty += 1
                #         color = (0, 0, 255)
                #         lane_points.append((current_x, current_y))
                #         if consecutive_empty >= self.max_empty_windows:
                #             lane_points = lane_points[:-self.max_empty_windows]
                #             break
        ...
        while True:
            ...
            if nonzero_window is not None and len(nonzero_window) >= min_pixel_threshold:
                ...
                if consecutive_empty >= self.max_empty_windows:
                    # max empty가 연속되면 탈출 (빈 window들은 그대로 저장함)
                    break

            cv2.rectangle(out_img, (win_x_low, win_y_low), (win_x_high, win_y_high), color, 2)
            current_y -= window_height

        # 루프 종료 후, lane_points를 y좌표 기준으로 정렬 (오름차순)
        lane_points_sorted = sorted(lane_points, key=lambda p: p[0][1])

        # 리스트의 양 끝이 non-empty("n")인 구간만 남김
        first_n_idx = None
        last_n_idx = None
        ...
        if first_n_idx is not None and last_n_idx is not None:
            ...
            # 만약 non-empty point가 하나도 없다면 전체 리스트를 사용하거나 별도 처리
            lane_points_final = []

        # flag는 제거하고 좌표만 추출
        lane_points_final = [pt for pt, flag in lane_points_final]

        return lane_points_final, out_img

        # fallback: lane_points가 비어있으면 해당 영역 중앙 사용
        # if not lane_points:
        ...
        #     color = (255, 0, 0)
        #     cv2.rectangle(out_img, (win_x_low, win_y_low), (win_x_high, win_y_high), color, 2)
        #     lane_points.append(((x_start + x_end) // 2, start_y))
    ...
    def process_occupancy_grid(self, msg: OccupancyGrid):
        ...
        # 왼쪽과 오른쪽 영역에서 시작점은 각각 해당 영역(절반)으로부터 구함.
        left_lane_points, left_vis = self.sliding_window_lane_detection_region(processed_img, 'left',
                                                                                self.window_width,
                                                                                self.window_height,
                                                                                self.min_pixel_threshold)
        right_lane_points, right_vis = self.sliding_window_lane_detection_region(processed_img, 'right',
                                                                                self.window_width,
                                                                                self.window_height,
                                                                                self.min_pixel_threshold)
